"""Bounded Qwen3.5-2B LoRA SFT for causal-memory maze planning data."""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run bounded Qwen3.5 LoRA SFT on causal maze-memory data.")
    parser.add_argument("--model-dir", type=Path, default=PROJECT_ROOT / "models" / "qwen3_5_2b")
    parser.add_argument(
        "--dataset", type=Path, default=PROJECT_ROOT / "assets" / "datasets" / "maze_sft_memory_smoke_v1.jsonl"
    )
    parser.add_argument("--run-name", default="qwen3_5_2b_maze_memory_sft_smoke_v1")
    ...
    if not args.model_dir.is_dir() or not args.dataset.is_file():
        raise FileNotFoundError("model directory or dataset is missing")
    ...
    run_dir = args.output_dir or PROJECT_ROOT / "runs" / "qwen35_sft" / f"{timestamp}_{args.run_name}"
    ...
    if args.resume_adapter:
        if not args.resume_adapter.is_dir():
            raise FileNotFoundError(args.resume_adapter)
        model = PeftModel.from_pretrained(model, args.resume_adapter, local_files_only=True, is_trainable=True)
    else:

...
### Translation-envelope recovery — 2026-09-02

- The final 300-step turn-hold-centre candidate ran continuously through the prior turn-drift boundary and reached the exit before the blue checkpoint. It then correctly backtracked rather than stopping early, but macro 160 fell short solely on cross-track (`0.955m` versus the hard `0.900m` limit) after already reaching required longitudinal travel. No fall/contact/timeout occurred; it is retained as `...turnholdcenter...dev300_v1.json`, 159 completed macros / 160 events, `physical_macro_incomplete`.
- Rather than loosening the 0.90m rule, the executor now has opt-in translation-envelope recovery. When a move/backtrack has enough longitudinal progress and only a bounded cross-track excess, it centre-seeks within the target cell, restores the motion heading, and advances only if the original longitudinal/cross-track/yaw gates all hold. Parameters are fully logged. This extends the same measured-pose principle already used for turn recovery.
- Source compilation plus CPU regression are `24 passed`. Next physical gate uses turn-hold-centre plus translation-envelope recovery; it first needs a bounded development pass before another full 300-step live claim.

### Translation-reorientation boundary — 2026-09-02

- The 180-step translation-recovery gate reached its target macro-160 translation boundary but stopped safely: centre recovery reduced cross-track error to `0.8992m` (within the unchanged `0.900m` limit), yet the fixed 400-tick heading reorientation did not meet the yaw threshold. The report is retained as a 159-complete-macro physical-macro-incomplete result, not a success.
- The next correction reuses the previously demonstrated turn-recovery sequence (48 neutral ticks plus up to 800 measured walking-turn ticks) for this reorientation. It recomputes longitudinal progress, cross-track error, centre error and yaw after the reorientation/recovery before allowing logical movement. This avoids treating pre-reorientation measurements as proof of post-reorientation state.
- Source compilation plus CPU regression are `24 passed`. Next step is a versioned 180-decision physical gate with the same hard collision/cross-track thresholds and the recovered reorientation path.

### BACKTRACK heading contract correction — 2026-09-02
...
### Full live semantic closure — 2026-09-02

- `artifacts\\h1\\qwen35_h1_live_frontier_exitroute_fullrecovery_seed657_cell48_dev300_v3.json` is the first accepted end-to-end live trace: 287 completed physical macros before semantic completion, blue checkpoint reached, then the robot traversed to the previously observed exit and stopped. The report has `final_logical_success=true`, final logical `(8,8)`, and SHA-256 `24E1F87967B2A4C12F463233FB3503994BCE34FE04830FE7F6794D6E443E4DB0`.
- The first v2 full run is retained as a valid diagnostic failure (all 300 physical macros complete, blue reached, but no semantic stop). Root cause was a guard-priority bug: when Qwen independently proposed the same known-exit `MOVE_FORWARD`, the later revisit rule could replace it with a turn. `guard_action` now always returns the observed-exit route before lower-priority revisit/frontier logic; the new regression verifies the matching-proposal case. Compilation plus CPU regression: `24 passed`.
- V3 route evidence is continuous and not replayed: after blue, it issued 46 `route_known_exit_after_checkpoint` actions across executed transitions, reached `(8,8)`, then emitted `goal_reached` → `STOP`. This is live Qwen3.5 plus local-memory guard and physical H1; it must be described as a hybrid executor, not pure LLM.
- Next: record versioned camera evidence from this successful trace/live setup, compose the evidence-first 8–12 minute film, then run stream/frame/audio/subtitle QC before final delivery.

### Final film v1 — 2026-09-02

- Final output: `artifacts\\video\\llm_h1_maze_final_film_v1.mp4`, 21,469,161 bytes, 489.20 seconds (8:09). It contains H.264 1280×720/30fps video, AAC mono audio, and a `mov_text` subtitle stream. Its source map and truth labels are stored in `artifacts\\video\\llm_h1_maze_final_film_v1.json`; the extracted subtitle source is `artifacts\\video\\llm_h1_maze_final_film_v1.srt`.
- New visual evidence includes `live_h1_success_trace_visualization_v1.mp4`, a 287-second report-derived visualization of the accepted live H1 trace. It is labelled in-frame as a **log visualization, not camera footage**. Start/middle/terminal frames were inspected: the terminal card shows event 287/287, `STOP`, `goal_reached`, and `蓝点 → 出口 → STOP，成功`.
- QC: `ffprobe` confirms H.264/AAC/mov_text streams and 489.20-second duration. `blackdetect` found no black segment at the configured 0.2-second threshold; `silencedetect` found no >=2-second silence under -45dB. Subtitles extract cleanly and finish at 08:09.183. The visual film keeps physical-camera footage separately labelled from report-derived visualizations; no frozen visualization is presented as live Qwen inference.
- Camera replay caveat: attempting a new visible-D3D12 replay of the success trace crashed during Isaac Sim/viewport initialization with a Windows access violation before any output artifact was written. It is not used as evidence; existing validated physical-camera bridge footage remains separately labelled. Recovery path: restart Isaac Sim, then attempt the camera path only after a minimal camera probe succeeds.

### Silent annotation edit — 2026-09-02

- User requested no voiceover. Final preferred output is `artifacts\\video\\llm_h1_maze_final_film_v2_silent.mp4`: 489.20 seconds (8:09), 14,498,970 bytes, H.264 1280×720/30fps **video only**. There is no audio or subtitle stream; Chinese annotations are burned into the picture at key nodes.
- Editorial reorder: first show the validated physical H1 bridge camera clip, then immediately show the full accepted live-H1 trace visualization. Blue checkpoint, observed-edge exit recovery, and `goal_reached → STOP` are all explicitly annotated. “Learning” is labelled as offline training/executor iteration; no claim says that H1 learns online while walking.
- Visual QC sampled the physical bridge, early decision panel, blue checkpoint-recovery panel and final STOP panel. All text is legible and no black segment was detected at the 0.2-second check threshold.
